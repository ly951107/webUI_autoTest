...
class testDiscuz1(BasePage):
    ...
    default_module = (By.CSS_SELECTOR, (".bm_c tr:nth-last-child(3) h2 a"))              #默认模块
    ...
    name_assert=(By.CSS_SELECTOR,"#um > p:nth-child(2) > strong > a")
    default_module_assert=(By.CSS_SELECTOR,"#ct > div > div.bm.bml.pbn > div > h1 > a")
    post_assert=(By.CSS_SELECTOR,"#thread_subject")
    reply_post_assert=(By.XPATH,"//*[@id='postmessage_154']")
    logout_assert=(By.CSS_SELECTOR,"#ls_username")
    #论坛实站1   # 用户登录
    def adminlogin(self,content1,content2):
        ...
        name=self.HuoQuText(*self.name_assert)
        return name
    def defaultModule(self):                                #点击登录默认模块
        self.click(*self.default_module)
        default_module_assert=self.HuoQuText(*self.default_module_assert)
        return default_module_assert
    def newModule(self):                                   #点击登录新模块
        time.sleep(3)
